...
class CollisionDetection {
   public:
    ...
    // AABB bounding mesh volume for complex meshes
    RigidObject getAABB(RigidObject & obj) {
        Eigen::MatrixXd V;
        Eigen::MatrixXi F;
        obj.getMesh(V, F);
        ...
        float xmin = minCoord[0];
        float ymin = minCoord[1];
        float zmin = minCoord[2];
        float xmax = maxCoord[0];
        float ymax = maxCoord[1];
        float zmax = maxCoord[2];

        Vnew.resize(8, 3);
        Vnew << xmin, ymin, zmin, xmax, ymin, zmin, xmin, ymax, zmin, xmax, ymax, zmin, xmin, ymin, zmax, xmax, ymin, zmax, xmin, ymax, zmax, xmax, ymax, zmax;

        Fnew.resize(12, 3);
        Fnew << 0, 2, 1, 2, 3, 1, 1, 3, 5, 3, 7, 5, 2, 6, 3, 6, 7, 3, 5, 7, 4, 7, 6, 4, 4, 6, 0, 6, 2, 0, 0, 4, 1, 4, 5, 1;

        RigidObject newObj;
        newObj.setMesh(Vnew, Fnew);
        newObj.setType(ObjType::DYNAMIC);
        newObj.setMass(1.0);

        newObj.setInertia(newObj.getMass() * 2.0 / 6.0 * Eigen::Matrix3d::Identity());
        newObj.reset();
    ...
    Eigen::Vector3d getCentre(RigidObject & obj) {
        Eigen::MatrixXd V;
        Eigen::MatrixXi F;
        obj.getMesh(V, F);
        ...
        Eigen::Vector3d centre = (minCoord + maxCoord) / 2;
